...
# 정보성 물체: 위험하지 않은 실내 물건 → "위험!"/"멈추세요" 없이 위치만 안내
# 이유: 키보드 0.5m 앞에 "위험!"은 경고 피로 유발 + 신뢰도 하락
_INFO_ONLY_KO = {
    "키보드", "마우스", "모니터", "TV", "텔레비전", "노트북",
    "휴대폰", "폰", "스마트폰", "태블릿",
    "컵", "머그컵", "병", "캔", "접시",
    "책", "공책", "잡지", "서류", "파일",
    "시계", "손목시계", "리모컨", "충전기", "이어폰", "헤드폰",
    "마우스패드", "스피커", "프린터", "키패드",
}
...
def _primary(obj: dict, abs_clock: str) -> str:
    ...
    is_info    = name in _INFO_ONLY_KO

    # ── 정보성 물체: 키보드·마우스·TV 등 — 위치만 안내, 위험 어조 없음 ──
    if is_info:
        return f"{direction} 가까이에 {name}{ig} 있어요."
