        ...
        └─ build_sentence()                │
            (한국어 문장 생성)             │
                    ...
                    └──────────┬───────────┘
                               ...
                               │ alert_mode: "critical" | "silent"
                               ▼
                    [Android TTS 음성 출력]  ← critical: 1.25× 속도로 즉각
                     또는 음성(일반 속도)    ← beep 제거: 비프음 → 음성으로 교체
...
| ~~텍스트~~ | ~~"글자 읽어줘"~~ | ~~ML Kit OCR~~ → **제거됨** (legacy) |
| ~~바코드~~ | ~~"바코드"~~ | ~~ML Kit 바코드 스캔~~ → **제거됨** (legacy) |
| 색상 | "색깔 알려줘" | HSV 색상 분석 |
| 밝기 | "어두워" | 조도 센서 값 읽기 |
...
계산 예시:
  ...
  → 0.06  (낮은 위험 → silent, 2.5m 이내만 critical)
```
...
- alert_mode(critical/silent)가 어떻게 결정되는지 (2.5m 이내 critical, 이상 silent)

### 신유득 (Vision/ML)
- 방향 9구역이 어떻게 결정되는지 (cx_norm)
- 위험도 점수 계산 공식
- 계단을 YOLO 탐지에서 제외한 이유 (오탐률 → Depth 맵 대체)
- conf threshold를 왜 0.50으로 설정했는지

### 신유득 (Depth) / 문수찬 (Voice)
- Depth V2가 "정확한 미터"가 아닌 이유 (상대 깊이)
- 왜 bbox 면적 하위 30% 값을 쓰는지 (안전 우선)
- 계단 감지를 Depth 맵으로만 하는 이유 (YOLO 오탐률 높음)
