...
이 문서는 `server_db/main.py`의 **Voice Guard API**를 Supabase Postgres에 연결하고 실행하는 과정을 정리한 가이드입니다.
...
```bat
cd /d "C:\Users\User\OneDrive\Desktop\AI 휴먼 1차 프로젝트_서버_db\server_db"
...
- 문서 페이지: `http://127.0.0.1:8002/docs`
- 헬스 체크: `http://127.0.0.1:8002/health`

정상이라면 Swagger 제목이 **`Voice Guard API`** 로 표시됩니다.

## 6) DB 테이블 준비

현재 `server_db/main.py`는 서버 시작 시 아래 테이블을 자동 생성합니다.

- `items`
- `lane_wear_results`

즉, 기본적인 테이블 준비는 별도 수동 작업 없이도 동작합니다.
...
`/lane_wear_infer`와 `/blur`를 테스트하려면 YOLO 모델 파일 경로를 환경변수로 지정해야 합니다.
