...
```
Android 앱 (Kotlin)
 └─ CameraX 캡처 → yolo26n_float32.tflite (TFLite) → 탐지 결과 JSON
          ↓
...
| 영역 | 기술 |
|---|---|
| Android | Kotlin, CameraX, TensorFlow Lite, OkHttp |
| 서버 | Python 3.10, FastAPI, Uvicorn |
| 비전 | Android TensorFlow Lite 기반 YOLO26n |
...
2. `assets/yolo26n_float32.tflite` 파일 확인 (온디바이스 추론용)
...
### yolo26n_float32.tflite (온디바이스)

- 기반: YOLO26n, TensorFlow Lite FP32 변환
- 폰 단독 실행 (TensorFlow Lite Android)
...
│       ├── TfliteYoloDetector.kt  # TensorFlow Lite 온디바이스 추론
