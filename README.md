...
| 서버 URL 입력 | 질문/색상/신호등 등 서버 기능 사용 가능 |
| 장애물/찾기 모드 | 서버 URL이 있어도 ONNX 우선 → 서버/ ngrok 장애 시에도 기본 탐지 유지 |
...
> 계단은 서버 연결 시 Depth 맵 12구역 분석으로 더 안정적으로 감지하며, Android 온디바이스에서는 `StairsDetector`가 보조 후보로 함께 합쳐집니다.
...
| **바운딩박스 오버레이** ⭐ | ✅ | 온디바이스 탐지 결과를 카메라 프리뷰 위에 실시간 시각화, 설정 또는 설정 버튼 길게 누르기로 디버그 표시 |
