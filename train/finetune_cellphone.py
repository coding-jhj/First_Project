...
EPOCHS       = 25          # 기존 지식 유지를 위해 짧게 (50에포크 이상이면 계단 지식 잊어버림)
IMGSZ        = 640         # YOLO 표준 입력 해상도
BATCH        = 12          # OOM 나면 8로 줄일 것
WORKERS      = 4           # 데이터 로더 스레드 수
PROJECT      = "runs/train"
NAME         = "cellphone_fix_v1"
DEVICE       = 0           # GPU 0번 (RTX 5060)
...
def main():
    ...
    # 이전 파인튜닝 결과 위에 이어서 학습하므로 계단 탐지 성능 유지됨
    ...
    results = model.train(
        ...
        lr0=5e-5,      # finetune.py의 1e-4보다 낮게 — 기존 지식 보존
        lrf=0.01,      # 최종 lr = lr0 * lrf (lr 감소 범위)
        cos_lr=True,   # 코사인 스케줄: 학습 후반부에 lr 천천히 감소
        label_smoothing=0.05,  # 레이블 스무딩: 과적합 방지
        augment=True,          # 데이터 증강: 각도·밝기 변환 등
    ...
    # 기존 indoor 모델 백업 후 덮어쓰기 — 롤백 가능하도록 백업 유지
