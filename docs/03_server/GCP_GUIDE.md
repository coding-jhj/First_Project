...
## Compute Engine 무료 등급 — e2-micro 설명

GCP 콘솔에서 "무료로 e2-micro VM 1개/월" 이라고 나오는데, 이게 뭔지 정리.

### 항목별 설명

| 항목 | 내용 |
|------|------|
| **e2-micro VM 1개** | RAM 1GB, CPU 0.25코어짜리 가상 컴퓨터 1대 |
| **무료 리전** | 미국 3곳만 — us-west1, us-central1, us-east1 (서울 해당 없음) |
| **30GB 디스크** | VM에 붙이는 하드디스크 30GB 무료 |
| **아웃바운드 1GB/월** | 서버에서 클라이언트로 전송하는 데이터 1GB까지 무료 |
| **"시간별 제한"** | 월 720시간(24h × 30일) 안에서 무료 — VM 1대 한 달 내내 켜도 딱 맞음 |
| **GPU** | **완전 유료** — VM에 GPU 붙이는 순간 바로 과금 시작 |

### VoiceGuide에 쓸 수 있어?

| 항목 | 현실 |
|------|------|
| RAM 1GB | YOLO11m 로드만 해도 2~3GB 필요 → **부족** |
| CPU 0.25코어 | 추론 너무 느림 → **실사용 어려움** |
| GPU 무료? | **완전 유료**, 붙이는 순간 과금 시작 |
| 리전 | 미국만 → 한국에서 레이턴시 높음 |

> **결론: 무료 e2-micro는 VoiceGuide 서버용으로 부적합.**
> 발표용으로는 **Cloud Run (무료 한도 내)** 쓰는 게 훨씬 낫고,
> $300 크레딧 받아서 쓰는 걸 추천.
