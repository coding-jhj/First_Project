...
> 2026년 4월 26일 | 역할명이 아닌 실제 업무량 기준으로 재분배
...
| 업무 | 설명 |
|---|---|
| YOLO11m 실내외 전체 탐지 | 44클래스 + 계단 파인튜닝 (mAP50=0.992) |
...
| 테스트 이미지 수집 | 실내외 41개 카테고리 (차량/동물/실내가구/계단 등) |
...
> 조장(정환주)이 전체 파이프라인을 선행 구현. 팀원들은 각자 모듈 검증·보완 담당.
...
| 5/1~5/7 | 서버 문서화 + 안정화 | 최종 인식률 데이터 정리 | 함수 최종 완성 | PPT 완성 + 발표 대본 |
