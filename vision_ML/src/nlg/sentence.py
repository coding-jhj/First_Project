...
routes.py에서 호출되는 공개 함수:
  ...
  build_hazard_sentence()  — 바닥 위험(낙차 등) 최우선 안내
...
def get_alert_mode(obj: dict, is_hazard: bool = False) -> str:
    ...
    if is_hazard:                          # 바닥 위험 — 낙상 가능성이므로 거리 무관 경고
        return "critical"
...
def _primary(obj: dict, abs_clock: str) -> str:
    ...
    is_critical = (name in _CRITICAL_KO or is_vehicle
                   or (is_animal and dist_m < 3.0) or is_hazard)
...
def build_hazard_sentence(
    ...
    """
    낙차·턱 등 바닥 위험을 최우선으로 안내.

    hazard.risk >= 0.7이면 다른 물체 안내 없이 위험만 말함.
    이유: 긴급 구간에서 부가 안내가 방해될 수 있음.
    ...
    # 고위험(0.7+) 또는 주변 물체 없으면 위험 메시지만
    if h_risk >= 0.7 or not objects:
        return h_msg

    # 저위험 + 주변 물체 있으면 둘 다 안내
