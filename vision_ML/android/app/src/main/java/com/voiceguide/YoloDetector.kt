...
class YoloDetector(context: Context) {
    ...
    private fun postProcess(
        ...
        for (i in 0 until numDet) {
            ...
            // YOLO ONNX 출력 bbox 포맷은 모델에 따라 다를 수 있음:
            // - (cx, cy, w, h) in pixels (letterbox 640 기준)
            // - (x1, y1, x2, y2) in pixels (letterbox 640 기준)
            // - 위 값들이 0~1 정규화로 나오는 경우도 있음
            //
            // 포맷이 맞지 않으면 오버레이에서 "세로 줄"처럼 찢어진 박스가 생김.
            // → 값 범위/관계로 자동 판별 후 (cx,cy,w,h)로 통일한다.

            var a = buf.get(0 * numDet + i)
            var b = buf.get(1 * numDet + i)
            var c = buf.get(2 * numDet + i)
            var d = buf.get(3 * numDet + i)

            // 1) 정규화(0~1)로 출력된 경우 → 640px 기준으로 복원
            val maxAbs = maxOf(kotlin.math.abs(a), kotlin.math.abs(b), kotlin.math.abs(c), kotlin.math.abs(d))
            if (maxAbs <= 2.0f) {
                a *= inputSize
                b *= inputSize
                c *= inputSize
            ...
            // 2) xyxy vs cxcywh 판별
            // xyxy라면 (x2>x1, y2>y1) 관계가 성립하고, 좌표가 0~640 근방에 존재
            val looksLikeXyxy = (c > a) && (d > b) &&
                (a >= 0f && b >= 0f && c <= inputSize.toFloat() * 1.2f && d <= inputSize.toFloat() * 1.2f)
            ...
            // letterbox된 640x640 공간의 픽셀값 → 패딩 제거 후 원본 이미지 [0,1] 비율로 변환
            ...
            // 너무 작거나 비정상 박스 제거 (오버레이 찢김 방지)
            if (w <= 0.001f || h <= 0.001f || w > 1.2f || h > 1.2f) continue
