...
| 멤버 | 브랜치 | 담당 모듈 | 완성 산출물 |
|------|--------|---------|-----------|
| **A** | `feature/android` | Android 앱 | 카메라 캡처 → 서버 전송 → TTS 재생 작동하는 앱 |
| **B** | `feature/api` | FastAPI 서버 | `/detect` API + DB + 모듈 통합 |
| **C** | `feature/vision` | YOLO + 방향/위험도 | `detect_and_depth()` 함수 |
| **D** | `feature/voice` | Depth V2 + STT/TTS | `detect_and_depth()` 함수 (C와 협업) |
...
│
├── data/
│   └── test_images/       시나리오별 테스트 이미지 (C가 수집)
│
├── results/               인식률 실험 결과
│   └── eval_log.md
│
├── app.py                 MVP Gradio 데모 (B가 통합)
├── requirements.txt
└── .env.example
```
...
```bash
git clone https://github.com/[팀레포]/voiceguide.git
cd voiceguide
